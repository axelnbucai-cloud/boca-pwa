...
export default function BocaJuniorsApp() {
  ...
  // Mapear ID de liga a estilo
  const getCompetitionStyle = (leagueId, leagueName) => {
    const id = parseInt(leagueId);
    if (id === LIBERTADORES_ID) return { name: 'Copa Libertadores', color: '#0a8c3a' };
    if (id === SUDAMERICANA_ID) return { name: 'Copa Sudamericana', color: '#d97706' };
    if (id === COPA_ARGENTINA_ID) return { name: 'Copa Argentina', color: '#8c0d0d' };
    if (id === LIGA_ID) return { name: 'Liga Profesional', color: '#0d4d8c' };
    return { name: leagueName || 'Partido', color: '#0d4d8c' };
  };

  // Helper para llamar API-Football con la key
  const apiFootballFetch = async (endpoint, timeout = 10000) => {
    ...
    try {
      ...
      // API-Football devuelve errores dentro del objeto
  ...
  const loadFixture = async () => {
    ...
    if (!API_FOOTBALL_KEY || API_FOOTBALL_KEY === 'PEGA_TU_API_KEY_ACA') {
      setFixtureError('API key no configurada. Editá src/config.js');
      setFixtureLoading(false);
      return;
    }
    try {
      // Próximos 10 partidos de Boca (cualquier competición)
      const events = await apiFootballFetch(
        `/fixtures?team=${BOCA_TEAM_ID}&next=10`
      );

      const mapped = events.map((ev) => {
        const isHome = ev.teams.home.id === BOCA_TEAM_ID;
        const compStyle = getCompetitionStyle(ev.league.id, ev.league.name);
        const dateObj = new Date(ev.fixture.date);
        return {
          id: ev.fixture.id,
          competition: compStyle.name,
          competitionColor: compStyle.color,
          date: dateObj.toISOString().split('T')[0],
          time: dateObj.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Argentina/Buenos_Aires' }),
          home: ev.teams.home.name,
          homeLogo: ev.teams.home.logo,
          away: ev.teams.away.name,
          awayLogo: ev.teams.away.logo,
          venue: ev.fixture.venue?.name || (isHome ? 'La Bombonera' : 'A confirmar'),
          city: ev.fixture.venue?.city || '',
          isHome,
          status: ev.fixture.status?.short || 'NS',
          matchday: ev.league.round || ev.league.season || 'Temporada 2026'
        };
      });
      ...
      if (mapped.length > 0) {
        ...
        setFixtureError('No hay partidos próximos publicados');
      }
    } catch (e) {
      console.error('Error cargando fixture:', e);
      setFixtureError(`Error: ${e.message}`);
    } finally {
      setFixtureLoading(false);
    }
  };
  ...
  const loadResults = async () => {
    ...
    if (!API_FOOTBALL_KEY || API_FOOTBALL_KEY === 'PEGA_TU_API_KEY_ACA') {
      setResultsError('API key no configurada. Editá src/config.js');
      setResultsLoading(false);
      return;
    }
    try {
      // Últimos 10 partidos (jugados) de Boca
      const events = await apiFootballFetch(
        `/fixtures?team=${BOCA_TEAM_ID}&last=10`
      );

      const mapped = events.map((ev) => {
        const isHome = ev.teams.home.id === BOCA_TEAM_ID;
        const compStyle = getCompetitionStyle(ev.league.id, ev.league.name);
        const homeScore = ev.goals.home;
        const awayScore = ev.goals.away;
        let outcome = 'draw';
        ...
        // Detectar ganador
        const homeWinner = ev.teams.home.winner;
        const awayWinner = ev.teams.away.winner;
        ...
        return {
          ...
          // Marcadores parciales (HT y FT)
          halftimeHome: ev.score?.halftime?.home,
          halftimeAway: ev.score?.halftime?.away,
          venue: ev.fixture.venue?.name || '',
          city: ev.fixture.venue?.city || '',
          isHome,
          outcome,
          statusLong: ev.fixture.status?.long || 'Finalizado',
          statusShort: ev.fixture.status?.short || 'FT',
          matchday: ev.league.round || ev.league.season || ''
        };
      });
      ...
      setResultsError(`Error: ${e.message}`);
    } finally {
      setResultsLoading(false);
    }
  };
  ...
  // ============ TABLAS DE POSICIONES (API-Football) ============
  const LEAGUES = {
    liga: { id: LIGA_ID, name: 'Liga Profesional', color: '#0d4d8c' },
    libertadores: { id: LIBERTADORES_ID, name: 'Copa Libertadores', color: '#0a8c3a' }
  };
  ...
  const loadStandings = async (leagueKey) => {
    ...
    if (!API_FOOTBALL_KEY || API_FOOTBALL_KEY === 'PEGA_TU_API_KEY_ACA') {
      setStandingsError('API key no configurada. Editá src/config.js');
    ...
    try {
      ...
      // API-Football devuelve un array de leagues; cada league tiene standings
      // que es un array de grupos (cada grupo es un array de equipos)
      let groups = [];
      if (data.length > 0 && data[0].league?.standings) {
        const allStandings = data[0].league.standings;
        groups = allStandings.map((groupRows, idx) => {
          const groupName = groupRows[0]?.group || `Grupo ${idx + 1}`;
          return {
            name: groupName,
      ...
      if (groups.length === 0) {
        setStandingsError(`Tabla no disponible para ${league.name} ${SEASON}`);
      }

      setStandings(prev => ({ ...prev, [leagueKey]: groups }));
    } catch (e) {
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    {match.home === 'Boca Juniors' ? (
                      ...
                      <img src={match.homeLogo} alt="" style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {match.away === 'Boca Juniors' ? (
                      ...
                      <img src={match.awayLogo} alt="" style={{
                        width: 24, height: 24, objectFit: 'contain', flexShrink: 0
                      }} />
                    ) : (
                      <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{
                      fontSize: 13,
                      color: match.homeWinner === false ? colors.blanco + '88' : colors.blanco,
                      fontWeight: match.home === 'Boca Juniors' ? 800 : 600,
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
                      textOverflow: 'ellipsis'
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{
                      fontSize: 13,
                      color: match.awayWinner === false ? colors.blanco + '88' : colors.blanco,
                      fontWeight: match.away === 'Boca Juniors' ? 800 : 600,
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
                      textOverflow: 'ellipsis'
                ...
                {/* Info adicional: HT y estadio */}
                {(match.halftimeHome !== undefined && match.halftimeHome !== null) && (
                  <div style={{
                    marginTop: 8, paddingTop: 8,
                    borderTop: `1px solid ${colors.blanco}10`,
                    display: 'flex', justifyContent: 'space-between',
                    fontSize: 10, color: colors.blanco + '88', fontWeight: 500
                  }}>
                    <span>HT: {match.halftimeHome}-{match.halftimeAway}</span>
                    {match.venue && <span style={{
                      whiteSpace: 'nowrap', overflow: 'hidden',
                      textOverflow: 'ellipsis', maxLength: 100
                    }}>📍 {match.venue}</span>}
                  </div>
                )}
              </div>
  ...
  const StandingsView = () => {
    const currentGroups = standings[standingsLeague] || [];
    ...
    return (
      <div>
        ...
        {standingsLoading && currentGroups.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            <div style={{
              display: 'inline-block', width: 40, height: 40,
              border: `3px solid ${colors.grisMedio}`,
        ...
        {!standingsLoading && currentGroups.length === 0 && (
          <div style={{
            ...
            <div style={{ color: colors.blanco, fontSize: 13, fontWeight: 700, marginBottom: 6 }}>
              {standingsError || 'Tabla no disponible'}
            </div>
            <div style={{ color: colors.blanco + '88', fontSize: 11, marginBottom: 14, lineHeight: 1.5 }}>
              Es posible que la temporada todavía no esté publicada o que tu API key no tenga acceso.
            </div>
        ...
        {/* Render cada grupo (Argentina suele tener Zona A y B) */}
        {currentGroups.map((group, gIdx) => (
          <div key={gIdx} style={{ marginBottom: 16 }}>
            ...
            <div style={{
              ...
              <div style={{
                display: 'grid',
                gridTemplateColumns: '28px 1fr 26px 22px 22px 22px 32px 32px',
                gap: 4,
                padding: '10px 10px',
                background: colors.azulOscuro,
              ...
              {/* Rows */}
              {group.rows.map((row) => {
                const isBoca = row.teamId === BOCA_TEAM_ID;
                return (
                  <div
                    key={row.teamId}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                      {isBoca ? (
                        ...
                        <img src={row.teamLogo} alt="" style={{
                    ...
                    <div style={{
                      ...
                      color: row.goalDiff > 0 ? '#22c55e' : row.goalDiff < 0 ? '#ef4444' : colors.blanco + 'cc',
                      fontWeight: 700
                    }}>
        ...
        {currentGroups.length > 0 && (
          <div style={{
            ...
            <span><b style={{ color: colors.blanco }}>DG</b> Diferencia</span>
